...
package org.isarnproject.collections.mixmaps.redblack
...
package tree {
  ...
  trait Node[K] extends Serializable {
    ...
    private[mixmaps] final def insert(d: Data[K]): Node[K] = blacken(ins(d))

    /** Delete the key from the tree */
    private[mixmaps] final def delete(k: K): Node[K] = this
